...
find	proc
		push	ebx
		push	eax
		XOR ebx, ebx	;先用bx算出需要比较的字符数
		XOR	ebp, ebp	;bp用于带出字符串长度
_f1:	inc ebx
		mov	bp, bx
...
locate	proc
		...
		push	edx
		push	ebx
		mov	esi, offset ga1	    ;在第一个网店寻找查询的商品名称
		XOR	ebp, ebp				;存储查找过的商品数目
		XOR	edx, edx
		mov dl, in_good+1		;存放输入的商品名称的长度
		XOR ebx, ebx
		cmp edx, 0			;只输入回车，回到功能1   0180
		jz  func1
l_1:	XOR ecx, ecx
		mov cl, [si+bx]
		XOR eax, eax
		...
		cmp ebx, edx
		jnz	l_1
		cmp BYTE PTR[esi+ebx], 0	;比较最后是否为0
		jnz l_2
		jmp	l_3
l_2:	add esi, 20
		inc ebp
...
l_3:	;si必然找到待寻找的那个商品
		...
		ret
		locate	endp
fresh  	proc	;更新si指向的商品的平均利润率 02b7
		push eax
		push ebx
		push ecx
		push edx
